...
function applyDataValidations(sheet, headers, sheetDocs, ss) {
  headers.forEach((header, index) => {
    ...
    // if (columnDoc.foreign_key) setForeignKeyValidation(ss, sheet, header, columnDoc.foreign_key, index + 1);
  });
}
...
function setForeignKeyValidation(ss, sheet, header, foreignKey, columnIndex) {
  ...
  var referenceRangeA1Notation = `${referenceSheetName}!${referenceHeader}2:${referenceHeader}${lastRow}`;
